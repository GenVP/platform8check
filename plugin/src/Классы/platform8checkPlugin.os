...
Процедура ВыполнитьПлатформеннуюПроверку(КаталогИсходныхКодов)
	...
	Если ПустаяСтрока(ФайлНастроек) Тогда
		Лог.Ошибка("Не указан файл настроек платформенной проверки");
		Возврат;
	КонецЕсли;

	Если КонфигураторХранилища = Неопределено Тогда
		Лог.Информация("Не было выгрузки новых версий из хранилища");
		Возврат;
	Конецесли;
	...
	Если НастройкиПроверки["Wait"] <> Неопределено Тогда
		МенеджерПроверок.ВремяОжидания(НастройкиПроверки["Wait"]);
	КонецЕсли;
	Если НастройкиПроверки["MaxWait"] <> Неопределено Тогда
		МенеджерПроверок.МаксимальноеВремяОжидания(НастройкиПроверки["MaxWait"]);
	КонецЕсли;
	
	МассивКлючей = НастройкиПроверки["Keys"];
	Если МассивКлючей <> Неопределено Тогда
		НомерПроверки = 1;
		Для Каждого КлючиПлатформеннойПроверки Из МассивКлючей Цикл
			МожноИспользоватьБазуИсточника = (НомерПроверки = МассивКлючей.Количество());
			МенеджерПроверок.НачатьПроверку(КлючиПлатформеннойПроверки, МожноИспользоватьБазуИсточника);
	...
	ГенераторОтчета.ВыгрузитьОшибкиВФайл(МенеджерПроверок.ТаблицаОшибок(),
		НастройкиПроверки["Project"], НастройкиПроверки["SonarQubeReport"]);
