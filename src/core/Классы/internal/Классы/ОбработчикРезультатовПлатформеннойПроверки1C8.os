...
Процедура ОбработатьФайлРезультата(Знач ФайлРезультатаПроверки) Экспорт
	...
	Сообщить(СтрШаблон("[%1] Начало Обработка ошибок для %2", ТекущаяДата(), ФайлРезультатаПроверки));
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		Если СтрНайти(ТекСтрока, "<<?>>") > 0 Тогда
			Сообщить(СтрШаблон(
				"ОШИБКА. Некорректный разбор файла. Найден <<?>> в новой строке ошибки. 
	...
	Сообщить(СтрШаблон("[%1] Окончание Обработка ошибок для %2", ТекущаяДата(), ФайлРезультатаПроверки));
...
Функция ФайлНеСодержитОшибок(ТекстФайлаПроверкиКонфигурации)
	...
	ПреодопределенныеРезультаты = Новый Массив;
	ПреодопределенныеРезультаты.Добавить(НСтр("ru='Ошибок не обнаружено'"));
	ПреодопределенныеРезультаты.Добавить(НСтр("ru='No errors found'"));
	ПреодопределенныеРезультаты.Добавить(НСтр("ru='Пользователь ИБ не идентифицирован'"));

	Для Каждого ТекстРезультата Из ПреодопределенныеРезультаты Цикл
		Если СтрСравнить(ТекстФайлаПроверкиКонфигурации, ТекстРезультата) = 0 Тогда
			Сообщить(СтрШаблон("[%1] %2", ТекущаяДата(), ТекстФайлаПроверкиКонфигурации));
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьПараметрыОшибки(ТекСтрока, СледСтрока)
	...
	Если ПозПервогоПробела = 0 Тогда
		Сообщить(СтрШаблон("ОШИБКА. Некорректный формат строки ошибки.
		|	Текущая строка %1", ТекСтрока));
	...
	Если НЕ ПустаяСтрока(СледСтрока) Тогда
		...
		Если ОписаниеОшибки.НачКолонка <> 0 Тогда
			Если ПозицияВопроса = 2 Тогда
				...
				ОписаниеОшибки.КонКолонка = ОписаниеОшибки.НачКолонка + СтрДлина(ОшибочныйКод);
			Иначе
...
Процедура ЗаполнитьПозициюВФайлеОшибки(ПараметрыОшибки, ОписаниеОшибки)
	...
	ПараметрыОшибки.ПутьКФайлу = ПреобразоватьОбъектВФайл(ПараметрыОшибки.ПутьКОбъекту, ЭтоМодуль);
...
Функция ПолучитьОписаниеОшибки(ПараметрыОшибки)
	...
	Если ОписаниеОшибки.ЭтоФорма Тогда
		Сообщить("ОШИБКА. Ошибки формы не поддерживаются. Пропуск ошибки.");
		Сообщить("	Ошибка = " + ПараметрыОшибки.ТекСтрока);
		Сообщить("	ПутьКОбъекту = " + ОписаниеОшибки.ПутьКОбъекту);
...
Функция ТаблицаКодовОшибок()
	...
	СтрокаОшибки = КодыОшибок.Добавить();
	СтрокаОшибки.Код = "V8_15";
	СтрокаОшибки.Шаблон = "Ожидается имя формального параметра";
	СтрокаОшибки.Статус = "Error";
	...
	Попытка
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(СтрокаЭлемента) Тогда
				Сообщить(СтрШаблон("Не найден элемент Имя: %1 в %2", ЭлементОбъекта, ПутьКОбъекту));
				Возврат "";
			КонецЕсли;
			...
			Если СоставОбъекта.Количество() > 3 Тогда
				...
				Иначе
					СтрокаЭлемента = ТипыОбъектов.Найти(ЭлементОбъекта, "Имя");
					Если НЕ ЗначениеЗаполнено(СтрокаЭлемента) Тогда
						Сообщить(СтрШаблон("Не найден элемент Имя: %1 в %2", СоставОбъекта[4], ПутьКОбъекту));
						Возврат "";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		КонецЕсли;
	Исключение
		Сообщить(СтрШаблон("Ошибка разбора объекта %1 (ЭтоМодуль=%2)", ПутьКОбъекту, ЭтоМодуль));
	КонецПопытки;
