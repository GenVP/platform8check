...
Перем КаталогИсходников; // Строка - каталог исходных кодов
Перем КонтекстИсточника; // Структура - поля КаталогБазы, Пользователь, Пароль - шаблонная файловая база источника 
...
Процедура ПриСозданииОбъекта()
	...
	ОбработчикРезультатов = Новый ОбработчикРезультатовПлатформеннойПроверки1C8(Лог);
...
Функция ИмяЛога() Экспорт
	...
	Возврат "oscript.lib.platform8check";
...
//
Функция КонтекстБазыИсточника(Знач ПарамИсходнаяБаза, Знач ПарамПользователь = "", Знач ПарамПароль = "") Экспорт

	КонтекстИсточника = Новый Структура;
	КонтекстИсточника.Вставить("КаталогБазы", ПарамИсходнаяБаза);
...
//
Функция КонфигураторБазыИсточника(Знач ПарамКонфигуратор) Экспорт

	СтруктураКонтекста = ПарамКонфигуратор.ПолучитьКонтекст();

	Если ПустаяСтрока(СтруктураКонтекста.КлючСоединенияСБазой) Тогда
		КаталогБазы = ПарамКонфигуратор.ПутьКВременнойБазе();

	ИначеЕсли НЕ СтрНачинаетсяС(СтруктураКонтекста.КлючСоединенияСБазой, "/F") Тогда
		ВызватьИсключение "В качестве источника проверки можно установить только файловую базу";
	Иначе
		КаталогБазы = Сред(СокрЛП(СтруктураКонтекста.КлючСоединенияСБазой), 3);
		// Удаление кавычек, если они были указаны в строке соединения
		Если СтрНачинаетсяС(КаталогБазы, """") И СтрЗаканчиваетсяНа(КаталогБазы, """") Тогда
			КаталогБазы = Сред(КаталогБазы, 2, СтрДлина(КаталогБазы) - 2);
		КонецЕсли;
	КонецЕсли;

	КонтекстИсточника = Новый Структура;
	КонтекстИсточника.Вставить("КаталогБазы", КаталогБазы);
	КонтекстИсточника.Вставить("Пользователь", СтруктураКонтекста.ИмяПользователя);
	КонтекстИсточника.Вставить("Пароль", СтруктураКонтекста.Пароль);
...
//
Функция КаталогИсходныхКодов(Знач ПарамКаталогИсходников) Экспорт
...
//
Функция СоздатьБазуИсточникаИзИсходныхКодов(КаталогБазы = "") Экспорт

	Если КаталогИсходников = Неопределено Тогда
		ВызватьИсключение "Не установлен каталог исходных кодов";
	КонецЕсли;
	...
	УправлениеКонфигуратором.УстановитьКонтекст(СтрШаблон("/F%1", КаталогБазы), "", "");
	УправлениеКонфигуратором.ЗагрузитьКонфигурациюИзФайлов(КаталогИсходников, "", "", Ложь);

	КонтекстБазыИсточника(КаталогБазы);
...
//
// Параметры:
//  ПарамВремяОжидания - Число - Время ожидания завершения платформенной проверки (секунд)
//
...
Функция НачатьПроверку(
	...
	Если КонтекстИсточника = Неопределено Тогда
		ВызватьИсключение "Не установлен контекст базы источника";
	КонецЕсли;
	...
	Если ЗапущенныеПроверки[КлючиПлатформеннойПроверки] <> Неопределено Тогда
		Лог.Ошибка("Для ключей %1 уже запущена платформенная проверка", КлючиПлатформеннойПроверки);
		Возврат ЭтотОбъект;
	КонецЕсли;
	...
	Лог.Отладка("[%1] Запуск проверки с ключами %2 для базы %3 и результатом в %3",
...
//
// Параметры:
//   ФильтрПроверки - Сценарий - объект реализующий одну или обе функции
//                               "ПроверитьФайл(ОтносительныйПутьКфайлу)", "ПроверитьОбъект(ПутьКОбъекту)"
//
Функция ДобавитьФильтр(ФильтрПроверки)  Экспорт

	ОбработчикРезультатов.ДобавитьФильтр(ФильтрПроверки);
...
//
Функция УдалитьВсеФильтры() Экспорт

	ОбработчикРезультатов.УдалитьВсеФильтры();
...
Функция ВозможнаОбработкаРезультата() Экспорт
	...
	Пока НЕ ФайлНайден Цикл
		Для Каждого КлючИЗначение Из ЗапущенныеПроверки Цикл
			...
			Если ОписаниеПроверки.Выполняется Тогда
				...
				ИначеЕсли ТекущаяДата() > ОписаниеПроверки.НачалоПроверки + ВремяОжиданияМакс Тогда
					Лог.Отладка("[%1] Превышение времени ожидания проверки с ключами %1", ТекущаяДата(), КлючИЗначение.Ключ);
		...
		Если НЕ ФайлНайден Тогда
			Лог.Отладка("[%1] Ожидание завершения проверок %2 с", ТекущаяДата(), ВремяОжидания);
...
Процедура ОбработатьРезультат() Экспорт
	...
	Для Каждого КлючИЗначение Из ЗапущенныеПроверки Цикл
		...
		Если НЕ ОписаниеПроверки.Выполняется Тогда
			...
			Лог.Отладка("[%1] Начало Обработка ошибок для ключей %2, файл результата %3",
			...
			Лог.Отладка("[%1] Окончание Обработка ошибок для ключей %2, файл результата %3",
				ТекущаяДата(), КлючИЗначение.Ключ, ОписаниеПроверки.ФайлРезультата);
...
Процедура ПоказатьКонтекстВРежимеОтладки()
	Лог.Отладка("КаталогБазы "+ КонтекстИсточника.КаталогБазы);
	Лог.Отладка("ИмяПользователя <"+ КонтекстИсточника.Пользователь + ">");
	Если ПустаяСтрока(КонтекстИсточника.Пароль) Тогда
		Лог.Отладка("Пароль не задан");
	Иначе
		Лог.Отладка(" Задан пароль "+ КонтекстИсточника.Пароль);
	КонецЕсли;
КонецПроцедуры
...
Функция СоздатьВременнуюКопиюБазыИсточника()
	...
	Лог.Отладка("[%1] Копирование базы из %2 в %3",
