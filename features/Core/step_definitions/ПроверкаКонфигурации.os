...
Функция ПолучитьСписокШагов(КонтекстФреймворкаBDD) Экспорт
	...
	ВсеШаги.Добавить("ЯУстанавливаюФильтрФайловИзПеременной");
...
Процедура ЯСоздаюОбъектУправлениеКонфигураторомДляВременнойБазыИСохраняюВПеременную(КаталогБазы, КонтекстБазы) Экспорт

	СтрокаСоединения = СтрШаблон("/F""%1""", КаталогБазы);
...
Процедура ЯЗагружаюКонфигурациюИзФайловВКаталогеВБазу(ИмяПеременнойИсходников, ИмяПеременнойБазы) Экспорт
	...
	Если ТипЗнч() = Тип("УправлениеКонфигуратором") Тогда
		УправлениеКонфигуратором = ВременнаяБаза;
	Иначе
		УправлениеКонфигуратором = Новый УправлениеКонфигуратором;
...
//Я устанавливаю фильтр файлов из переменной "ФильтрФайлов"
Процедура ЯУстанавливаюФильтрФайловИзПеременной(ИмяПеременнойФильтра) Экспорт

	МенеджерПроверок = БДД.ПолучитьИзКонтекста("МенеджерПроверок");
	ПутьКФайлуФильтра = БДД.ПолучитьИзКонтекста(ИмяПеременнойФильтра);
	МенеджерПроверок.УстановитьФильтр(Новый ФильтрФайловПоСписку(ПутьКФайлуФильтра));
